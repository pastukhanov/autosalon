...
public class FrameApp implements ApplicationVew {
    ...
    private void processList() {
        ...
        switch (state) {
            ...
            case CUSTOMER_MENU -> populateTable(autoSalon.getAllCustomers(),
                    customer -> new Object[]{customer.getId(), customer.getName(), customer.getAge(), customer.getGender()});
            case SALES_MENU -> populateTable(autoSalon.getAllSales(),
                    sale -> new Object[]{sale.getId(), sale.getCar().getBrand(), sale.getCar().getModel(), sale.getCustomer().getName(), sale.getCustomer().getId()});
        }
    }
    ...
    private void updateColumnNamesForState(ApplicationState state) {
        switch (state) {
            ...
            case SALES_MENU -> setColumnNames(new String[]{"Sale ID", "Car Brand", "Car Model", "Customer Name", "Customer ID"});
        }
    }
    ...
    private void showAddCarForm() {
        JDialog addCarDialog = new Forms(frame, autoSalon).getCarForm(null, null, null, null, false);
    ...
    private void showEditCarForm() {
        ...
        JDialog addCarDialog = new Forms(frame, autoSalon).getCarForm(car.getId(), car.getType(), car.getModel(), car.getBrand(), true);
        addCarDialog.setVisible(true);
    }
